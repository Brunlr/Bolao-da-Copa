...
def txtparalista(arquivo,li):     #função que adiciona os elementos txt para lista para não haver repetição
    with open(arquivo, 'rt') as file:
        lines = file.readlines()
        for line in lines:
            elements = line.strip().split(";")  #remover os espaços e "fatiar" a string conforme o ";"
            if len(elements)>1:
                li.append(elements)
    listaint(li)
    return li
